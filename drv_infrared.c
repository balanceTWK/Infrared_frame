...
#define PWM_DEV_NAME              "pwm4"        /* PWM name */
#define PWM_DEV_CHANNEL           3
#define INFRARED_SEND_HWTIMER     "timer15"     /* Timer name */
#define MAX_SEND_SIZE             1000

/* Infrared receiver configuration parameters */
#define INFRARED_RECEIVE_PIN      17            /* receive pin */
#define INFRARED_RECEIVE_HWTIMER  "timer16"     /* Timer name */

#define RECEIVE_HWTIMEER_SEC      0
#define RECEIVE_HWTIMEER_USEC     1000 * 1000

struct rt_device_pwm         *pwm_dev;
static volatile rt_uint32_t  infrared_send_buf[MAX_SEND_SIZE];
static rt_hwtimerval_t       timeout_s;
static rt_device_t           send_time_dev ;
static rt_device_t           receive_time_dev ;

volatile static rt_uint32_t diff_us;
volatile static rt_uint32_t receive_flag = 0x00000000;
...
rt_err_t infrared_send_init(void)
{
    ...
    send_time_dev = rt_device_find(INFRARED_SEND_HWTIMER);
    if (send_time_dev == RT_NULL)
    {
        LOG_E("hwtimer sample run failed! can't find %s device!", INFRARED_SEND_HWTIMER);
        return RT_ERROR;
    }
    ret = rt_device_open(send_time_dev, RT_DEVICE_OFLAG_RDWR);
    if (ret != RT_EOK)
    {
        LOG_E("open %s device failed!\n", INFRARED_SEND_HWTIMER);
        return ret;
    }
...
static void receive_pin_callback(void* param)
{
    ...
    if( (receive_flag & (1<<0)) )
    {
        ...
        if(rt_pin_read(INFRARED_RECEIVE_PIN) == PIN_HIGH)
        {
...
rt_err_t infrared_receive_init(void)
{
    ...
    rt_pin_mode(INFRARED_RECEIVE_PIN,PIN_MODE_INPUT_PULLUP);
    rt_pin_attach_irq(INFRARED_RECEIVE_PIN,PIN_IRQ_MODE_RISING_FALLING,receive_pin_callback,RT_NULL);
    rt_pin_irq_enable(INFRARED_RECEIVE_PIN,PIN_IRQ_ENABLE);

    receive_time_dev = rt_device_find(INFRARED_RECEIVE_HWTIMER);
    if (receive_time_dev == RT_NULL)
    {
        LOG_E("hwtimer sample run failed! can't find %s device!", INFRARED_RECEIVE_HWTIMER);
        return RT_ERROR;
    }
